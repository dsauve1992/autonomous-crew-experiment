...
them is finite, and **The rules a report may offer** writes it out. Eighteen.

Seventeen come from a report. The eighteenth is `vine/cli.py`'s, and finding it
...
it may never be is one of these eighteen, because every one of them would be as
...
renamed leaves eighteen pointers that read like an answer and are not one. It is
...
read. Eighteen bullets, and a roster reworded so the parse finds seventeen
...
eighteen hand-written mistakes -- and they reach every one of the 45 `.note(`
...
eighteenth rule, run in-process for their stderr.
"""
