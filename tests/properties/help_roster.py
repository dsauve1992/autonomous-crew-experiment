...
them is finite, and **The rules a report may offer** writes it out. Nineteen.

Eighteen come from a report. The nineteenth is `vine/cli.py`'s, and finding it
...
it may never be is one of these nineteen, because every one of them would be as
...
renamed leaves nineteen pointers that read like an answer and are not one. It is
...
read. Nineteen bullets, and a roster reworded so the parse finds eighteen
...
nineteenth rule, run in-process for their stderr.
"""
